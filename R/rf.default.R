...
"rf.default" <-
    ...
    if (importance) {
        ...
        if (classRF) {
            impout <- array(0.0, dim=c(p, nclass + 2, ntree))
            impSD <- array(0.0, dim=c(p, nclass + 1, ntree))
        } else {
            impout <- array(0.0, dim=c(p, 2, ntree))
            impSD <- matrix(0.0, p, ntree)
    ...
    if (classRF) {
        ...
        out <- list(call = cl,
                    ...
                    oob.times = oob.times,
                    classes = levels(y),
                    # TODO: store importance matrix in separate variable to maintain
                    # compatibility with randomForest objects
                    impmat = if (importance)
                    array(rfout$impout, dim=c(p, nclass+2, ntree),
                           dimnames = c(list(x.col.names,
                    ...
                    importance = if (importance)
                      matrix(rfout$impout[,,ntree], nrow=p, ncol=nclass+2,
                            dimnames = c(list(x.col.names,
                                              c(levels(y), "MeanDecreaseAccuracy",
                                                "MeanDecreaseGini"))))
                    else matrix(rfout$impout, ncol=1,
        ...
        out <- list(call = cl,
                    ...
                    importance = if (importance) array(rfout$impout, dim=c(p, 2,ntree),
                    dimnames=c(list(x.col.names,
                                  c("%IncMSE","IncNodePurity")), NULL)) else
