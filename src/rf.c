...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	...
	*at, *a, *b, *mind, *nind, *jts, *oobpair;
    ...
    tgini =      (double *) S_alloc(mdim*Ntree, sizeof(double));
    ...
    if (imp) {
        zeroDouble(imprt, (nclass+2) * mdim * Ntree);
        zeroDouble(impsd, (nclass+1) * mdim * Ntree);
    ...
    for (jb = 0; jb < Ntree; jb++) {
		...
		do {
			...
			F77_CALL(buildtree)(a, b, cl, cat, maxcat, &mdim, &nsample,
                                ...
                                bestsplit, bestsplitnext, tgini+mdim*(Ntree-1),
            ...
            memcpy(tgini+mdim*jb, tgini+mdim*(Ntree-1), sizeof(double)*mdim);
...
+								nrightimp[cl[n] - 1]++;
					...
					for (n = 0; n < nclass; ++n) {
						if (nout[n] > 0) {
                            ...
                            for(int a=jb;a<Ntree;a++){
                                imprt[m + n*mdim + (nclass+2)*mdim*a] += delta;
                                impsd[m + n*mdim + (nclass+1)*mdim*a] += delta * delta;
                            }
						}
					}
					...
					if (noutall > 0) {
                        ...
                        for(int a=jb;a<Ntree;a++){
                            imprt[m + nclass*mdim + (nclass+2)*mdim*a] += delta;
                            impsd[m + nclass*mdim + (nclass+1)*mdim*a] += delta * delta;
                        }
					}
				}
			}
    ...
    if (imp) {
        for(int a=0;a<Ntree;a++){
            for (m = 0; m < mdim; m++) tgini[m+mdim*a] /= (a+1);
            for (m = 0; m < mdim; ++m) {
                ...
                for (k = 0; k < nclass; ++k) {
                    av = imprt[m + k*mdim + (nclass+2)*mdim*a] / (a+1);
                    impsd[m + k*mdim + (nclass+1)*mdim*a] =
                    sqrt(((impsd[m + k*mdim + (nclass+1)*mdim*a] / (a+1)) - av*av) / (a+1));
                    imprt[m + k*mdim + (nclass+2)*mdim*a] = av;
                    /* imprt[m + k*mdim] = (se <= 0.0) ? -1000.0 - av : av / se; */
                }
                /* overall measures */
                av = imprt[m + nclass*mdim + (nclass+2)*mdim*a] / (a+1);
                impsd[m + nclass*mdim + (nclass+1)*mdim*a] =
                sqrt(((impsd[m + nclass*mdim + (nclass+1)*mdim*a] / (a+1)) - av*av) / (a+1));
                imprt[m + nclass*mdim + (nclass+2)*mdim*a] = av;
                imprt[m + (nclass+1)*mdim + (nclass+2)*mdim*a] = tgini[m+mdim*a];
            }
        }
    } else {
